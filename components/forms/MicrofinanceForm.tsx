...
export default function MicrofinanceForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status === 200) {
        setSubmitted(true);
        setFormData({ fullName: '', cnic: '', phone: '', businessType: '', estimatedCapital: '', address: '' });
        setDocument(null);
      }
    } catch (err) {
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-8 w-full max-w-3xl mx-auto">
      <h2 className="text-2xl font-semibold mb-6 border-b pb-2">Microfinance Support Form</h2>
      {submitted && <div className="bg-green-100 text-green-700 p-3 mb-4 rounded">Request submitted successfully!</div>}
      <form onSubmit={handleSubmit} className="space-y-5">
        {['fullName', 'cnic', 'phone', 'businessType', 'estimatedCapital', 'address'].map(field => (
          <div key={field}>
            <label className="block mb-1 font-medium capitalize">{field.replace(/([A-Z])/g, ' $1')}</label>
            <input
              type={field === 'estimatedCapital' ? 'number' : 'text'}
              name={field}
